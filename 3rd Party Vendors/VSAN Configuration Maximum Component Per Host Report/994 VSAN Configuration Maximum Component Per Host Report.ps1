...
foreach ($cluster in $clusviews) {
   if($cluster.ConfigurationEx.VsanConfigInfo.Enabled) {
      foreach ($vmhost in ($cluster.Host) | Sort-Object -Property Name) {
         $vmhosDomainew = Get-View $vmhost -Property Name,ConfigManager.VsanSystem,ConfigManager.VsanInternalSystem
         $vsanSys = Get-View -Id $vmhosDomainew.ConfigManager.VsanSystem
         $vsanIntSys = Get-View -Id $vmhosDomainew.ConfigManager.VsanInternalSystem
