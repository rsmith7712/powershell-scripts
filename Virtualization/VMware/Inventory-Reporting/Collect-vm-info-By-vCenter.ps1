...
#################################
#$vCenterList = Get-Content "PATH:c:\tmp\vCenter.txt"
...
foreach ($vCenter in $vCenterList){
	...
	Connect-VIServer -Server srvvcenterp.example.com -ErrorAction SilentlyContinue -WarningAction 0 | Out-Null
	If($? -Eq $True){
	...
	foreach ($vm in Get-VM) {
		...
		$report | Export-Csv "C:\tmp\$Export_VM_INFO" -NoTypeInformation -UseCulture
	...
	$ReportAllSnap  | Export-Csv "C:\tmp\$Snap_INFO" -NoTypeInformation -UseCulture

	Write-Host "Export All VM Information"
	$DCReport | Export-Csv "C:\tmp\$All_VM_Export" -NoTypeInformation -UseCulture
}
<#
	##############################
