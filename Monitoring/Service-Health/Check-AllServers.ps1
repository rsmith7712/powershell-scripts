...
############################################# Script Start
$Script:ServerGroups = @("Server Pilot","Server Systems","Server Systems 2","DMZ Servers")
...
ForEach($Group in $Script:ServerGroups){
...
$wsusList += $wsusImport | ForEach-Object{$_.Split(".") | Select-Object -First 1} #Removes FQDN, for testing.
}

# Gets service info only for the servers in this Server Group.
$WSUSservices = Import-CSV $servicesList
$WSUStargets = $WSUSservices.Where({$wsusList -contains $_.ServerName})

# Creates HTML Report and Adds Header Info
$report = "\\SERVER\SHARE\...\Check_WSUS_All_Services.htm"
If(Test-Path $report)
{
    Remove-Item $report -force
    New-Item $report -Type File
}
Add-HTML $report "Open" "All Servers" # Creates the beginning of the html report file
