<#	
	...
	===========================================================================
	 ...
	 Filename:     	Check-PostPatchingAllGroups.ps1
	===========================================================================
    .DESCRIPTION
        ...
        - NO MENU. Just checks all servers and emails to NetOps too.
...
Function Send-Email($HTML,$ServerGroup)
{
    ...
    $msg = New-Object System.Net.Mail.MailMessage $from, $to, $cc, $subject, $body
    $msg.isBodyhtml = $true
    $smtp.send($msg)
}
...
############################################# Script Start
$Script:ServerGroup = "All Server Groups"
...
$GroupList = @("Server Pilot","Server Systems","Server Systems 2","DMZ Servers")
$wsusImport = @()
ForEach ($Group in $GroupList){
$mygroup = $wsus.GetComputerTargetGroups() | Where-Object {$_.Name -eq $Group}
$wsusImporter = $mygroup.GetComputerTargets() | Select-Object -ExpandProperty FullDomainName
$wsusImporter = ($wsusImporter | ForEach-Object{$_.Split(".") | Select-Object -First 1}) #Removes FQDN, for testing.
$wsusImport += $wsusImporter
}
