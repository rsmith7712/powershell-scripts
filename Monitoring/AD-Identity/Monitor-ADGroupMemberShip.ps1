...
<#
.DESCRIPTION
	This script is monitoring group(s) in Active Directory and send an email when 
    someone is changing the membership.
...
.PARAMETER SearchRoot
    ...
    By default, the script searches the entire sub-tree of which SearchRoot is the 
    topmost object (sub-tree search). This default behavior can be altered by using 
...
.PARAMETER File
	Specify the File where the Group are listed. DN, SID, GUID, or Domain\Name 
...
.PARAMETER EmailTo
	Specify the Email Address(es) of the Destination. Example: fxcat@company.com

.PARAMETER EmailFrom
	Specify the Email Address of the Sender. Example: Reporting@company.com
...
.PARAMETER ExtendedProperty
	Specify if you want to add Enabled and PasswordExpired Attribute on members in 
    the report
...
.EXAMPLE
	...
	This will run the script against the group FXGROUP and send an email to 
    To@Company.com using the address From@Company.com and the server mail.company.com.
...
.EXAMPLE
	...
	This will run the script against the groups FXGROUP,FXGROUP2 and FXGROUP3 and send 
    an email to To@Company.com using the address From@Company.com and the 
...
.EXAMPLE
	...
	This will run the script against the group FXGROUP and send an email to 
    To@Company.com using the address From@Company.com and the server mail.company.com. 
    Additionally the switch Verbose is activated to show the activities of the script.
...
.EXAMPLE
	...
	This will run the script against the group FXGROUP and send an email to 
    Auditor@Company.com and Auditor2@Company.com using the address From@Company.com 
    and the server mail.company.com. Additionally the switch Verbose is activated to 
    show the activities of the script.

.EXAMPLE
	.\AD-GROUP-Monitor_MemberShip.ps1 -SearchRoot 'company.com/TEST/Groups' -Emailfrom Reporting@company.com -Emailto "Catfx@company.com" -EmailServer 0.0.0.0 -Verbose

	This will run the script against all the groups present in the CanonicalName 
    'company.com/TEST/Groups' and send an email to catfx@company.com using the address 
    Reporting@company.com and the server 0.0.0.0. Additionally the switch Verbose is 
    activated to show the activities of the script.

.EXAMPLE
	.\AD-GROUP-Monitor_MemberShip.ps1 -file .\groupslist.txt -Emailfrom Reporting@company.com -Emailto "Catfx@company.com" -EmailServer 0.0.0.0 -Verbose

	This will run the script against all the groups present in the file 
    groupslists.txt and send an email to catfx@company.com using the address 
    Reporting@company.com and the server 0.0.0.0. Additionally the switch Verbose is 
    activated to show the activities of the script.

.EXAMPLE
	.\AD-GROUP-Monitor_MemberShip.ps1 -server DC01.company.com -file .\groupslist.txt -Emailfrom Reporting@company.com -Emailto "Catfx@company.com" -EmailServer 0.0.0.0 -Verbose

	This will run the script against the Domain Controller "DC01.company.com" on all the 
    groups present in the file groupslists.txt and send an email to catfx@company.com 
    using the address Reporting@company.com and the server 0.0.0.0. Additionally 
    the switch Verbose is activated to show the activities of the script.

.EXAMPLE
	.\AD-GROUP-Monitor_MemberShip.ps1 -server DC01.company.com:389 -file .\groupslist.txt -Emailfrom Reporting@company.com -Emailto "Catfx@company.com" -EmailServer 0.0.0.0 -Verbose

	This will run the script against the Domain Controller "DC01.company.com" 
    (on port 389) on all the groups present in the file groupslists.txt and send 
    an email to catfx@company.com using the address Reporting@company.com and the server 
    0.0.0.0. Additionally the switch Verbose is activated to show the 
    activities of the script.

.EXAMPLE
	.\AD-GROUP-Monitor_MemberShip.ps1 -group "Domain Admins" -Emailfrom Reporting@company.com -Emailto "Catfx@company.com" -EmailServer 0.0.0.0 -EmailEncoding 'ASCII' -HTMLlog

	This will run the script against the group "Domain Admins" and send an email 
    (using the encoding ASCII) to catfx@company.com using the address Reporting@company.com 
    and the SMTP server 0.0.0.0. It will also save a local HTML report under 
    the HTML Directory.

.OPERATIONAL

    >>AD Query to Gather and Export Full Group Listings (in either TXT or CSV)
...
### RUN This for Domain Environment ###	
>>>>>>Loads Groups in from TEXT file; Format must be in NAME listing only (no other attributes)
    .\Monitor-ADGroupMembership.ps1 -server srv.example.com -file .\AD_DistributionGroups_NAME_Infrastructure_NoDLs.txt -Emailfrom Pirate@example.com -Emailto "admin2@example.com, SysAdmins@example.com" -EmailServer 0.0.0.0 -Verbose

    >>Targetted to Specific Groups
    .\Monitor-ADGroupMembership.ps1 -group "AppServerAdmin","Domain Admins" -Emailfrom Pirate@example.com -Emailto "admin2@example.com, user4@example.com" -EmailServer 0.0.0.0 -Verbose
