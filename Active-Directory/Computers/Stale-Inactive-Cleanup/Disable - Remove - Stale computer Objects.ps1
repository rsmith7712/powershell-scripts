...
$1year = (Get-Date).AddDays(-60) # The 60 is the number of days from today since the last logon.
$1y1m = (Get-Date).AddDays(-95)
...
#Get-ADComputer -Property Name,lastLogonDate -Filter {lastLogonDate -lt $1y1m} | Remove-ADComputer -WhatIf

Get-ADComputer -Property Name,lastLogonDate -Filter {lastLogonDate -lt $1y1m} | Export-Csv C:\temp\report-stale-objects.csv -NoTypeInformation