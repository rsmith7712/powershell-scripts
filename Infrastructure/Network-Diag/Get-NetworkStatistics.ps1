...
function Logging($pingerror, $Computer, $PortReport)
{
	$outputfile = "\\SERVER\SHARE\...\log_PortScanResults.csv";
	
	$timestamp = (Get-Date).ToString();
	
	$logstring = "Computer / Domain Status: {0}, {1}" -f $Computer, $PortReport;
    	            ...
    	            if($item[1] -notmatch '^\[::'){
                        ...
                        #If we are running showprocessnames, get the matching name
                            ...
                            if($showHostnames){
                                $tmpAddress = $null
                                try{
                                    if($remoteAddress -eq "0.0.0.0" -or $remoteAddress -eq "0.0.0.0"){
                                ...
                                try{
                                    ...
                                    if($localAddress -eq "0.0.0.0" -or $localAddress -eq "0.0.0.0"){
...
$ServerList = Get-Content "\\SERVER\SHARE\...\list_TestPortHostTargets.txt"

ForEach ($Server in $ServerList)
{
	Get-NetworkStatistics -State Listening | Format-Table -autosize 
