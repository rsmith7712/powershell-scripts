...
#>
param ( [Parameter(Mandatory=$false)] $To = "admin2@example.com", [Parameter(Mandatory=$false)] $From = "apc_alerts@example.com", $AttachmentSizeMB=$null, $MessageCount=2, $SecondsDelay=10 )
$messageParameters = @{
 Body = $null | ConvertTo-Html -Body "<H2> Detected an unauthorized user attempting to access the SNMP interface </H2>" | Out-String
 From = $from
 To = $to
 SmtpServer = @(Get-TransportServer)[0].Name.ToString()
 }
if ($AttachmentSizeMB) {
if ((Test-Path $Env:TMP\$($AttachmentSizeMB)apcreport.txt) -ne $true) {
